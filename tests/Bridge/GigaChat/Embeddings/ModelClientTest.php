...
use FM\AI\Platform\Bridge\GigaChat\Embeddings\ModelClient;
...
final class ModelClientTest extends TestCase
{
    ...
    public function testItIsExecutingTheCorrectRequest()
    {
        $resultCallback = static function (string $method, string $url, array $options): HttpResponse {
            ...
            self::assertSame('https://gigachat.devices.sberbank.ru/api/v1/embeddings', $url);
            self::assertSame('Authorization: Bearer api-key', $options['normalized_headers']['authorization'][0]);
            self::assertSame('{"model":"Embeddings","input":"test message"}', $options['body']);

            return new MockResponse();
        };
        $httpClient = new MockHttpClient([$resultCallback]);
        $modelClient = new ModelClient($httpClient, 'api-key');
        $modelClient->request(new Embeddings(), 'test message');
    }
    ...
    public function testItIsExecutingTheCorrectRequestWithArrayPayload()
    {
        $resultCallback = static function (string $method, string $url, array $options): HttpResponse {
            ...
            self::assertSame('https://gigachat.devices.sberbank.ru/api/v1/embeddings', $url);
            self::assertSame('Authorization: Bearer api-key', $options['normalized_headers']['authorization'][0]);
            self::assertSame('{"model":"Embeddings","input":"Hello"}', $options['body']);

            return new MockResponse();
        };
        $httpClient = new MockHttpClient([$resultCallback]);
        $modelClient = new ModelClient($httpClient, 'api-key');
        $modelClient->request(new Embeddings(), 'Hello');
    }

    #[TestWith(['https://gigachat.devices.sberbank.ru/api/v1/embeddings'])]
    public function testItUsesCorrectBaseUrl(string $expectedUrl)
    {
        ...
        $httpClient = new MockHttpClient([$resultCallback]);
        $modelClient = new ModelClient($httpClient, 'api-key',);
        $modelClient->request(new Embeddings(), ['input' => 'test message']);
    }
}