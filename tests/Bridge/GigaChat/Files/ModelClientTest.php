...
use FM\AI\Platform\Bridge\GigaChat\Files\ModelClient;
use FM\AI\Platform\Bridge\GigaChat\Files\Task;
...
final class ModelClientTest extends TestCase
{
    ...
    public function testItIsExecutingTheCorrectRequestWithDefaultValues(): void
    {
        $resultCallback = static function (string $method, string $url, array $options): HttpResponse {
            self::assertSame('POST', $method);
            self::assertSame('https://gigachat.devices.sberbank.ru/api/v1/files', $url);
            self::assertSame('Authorization: Bearer api-key', $options['normalized_headers']['authorization'][0]);
            self::assertSame(null, $options['body']);
    ...
    public function testItIsExecutingTheCorrectRequestWithArrayPayload(): void
    {
        ...
        $httpClient = new MockHttpClient([$resultCallback]);
        $modelClient = new ModelClient($httpClient, 'api-key');
        $modelClient->request(new GigaChat(), ['task' => Task::FILE_INFO]);
    }

    #[TestWith(['https://gigachat.devices.sberbank.ru/api/v1/files'])]
    public function testItUsesCorrectBaseUrl(string $expectedUrl): void
    {
        ...
        $httpClient = new MockHttpClient([$resultCallback]);
        $modelClient = new ModelClient($httpClient, 'api-key',);
        $modelClient->request(new GigaChat(), ['input' => 'test message']);
    }
}